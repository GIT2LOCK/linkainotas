...
export const requireLinkaiUser = createMiddleware({ type: "function" }).server(async ({ next }) => {
  ...
  const { getSupabaseServerClient } = await import("@/integrations/supabase/server-session.server");
